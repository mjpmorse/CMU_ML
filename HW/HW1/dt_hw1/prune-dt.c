...
/* In order to prune from the bottom up we must recursively call BEFORE
we check the node such that we are starting as far down the tree as possible
*/
  ...
  if ((acc_after-acc_before)>EPSILON) {
    ...
    *******************************************************************/
		...
		/* In order to prune from the top down we must recursively call AFTER
		we check the node if it survives we go one node down stream and try again
