...
double Entropy(int num_pos, int num_neg) {
  ...
  double entropy=0.0;
  double probabilityPos = (double) num_pos/
                        ( (double) num_pos + (double) num_neg);
  double probabilityNeg = 1. - probabilityPos;
  entropy = -1.*(probabilityPos*LogBase2(probabilityPos) +
                (probabilityNeg*LogBase2(probabilityNeg)) );
  return entropy;
}
